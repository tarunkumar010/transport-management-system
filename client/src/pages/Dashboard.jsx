import React, { useEffect, useState } from "react";
import API from "../services/api";

export default function Dashboard() {
  const [data, setData] = useState({
    vehicles: [],
    deliveries: [],
    orders: [],
  });

  const role = localStorage.getItem("role") || "Admin";
  const name = localStorage.getItem("name") || "User";

  useEffect(() => {
    Promise.all([
      API.get("/vehicles"),
      API.get("/deliveries"),
      API.get("/orders"),
    ]).then(([v, d, o]) =>
      setData({
        vehicles: v.data,
        deliveries: d.data,
        orders: o.data,
      })
    );
  }, []);

  const driverDeliveries = data.deliveries.filter(
    (d) => d.vehicle?.driverName === name
  );

  if (role === "Driver") {
    const currentDelivery = driverDeliveries[0];

    return (
      <div>
        <h2 className="text-3xl font-bold mb-6">Driver Dashboard</h2>

        {driverDeliveries.length === 0 ? (
          <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 p-6 rounded-2xl shadow">
            <h3 className="text-xl font-bold mb-2">
              No delivery assigned yet
            </h3>
            <p>
              Your assigned deliveries will appear here when admin assigns a
              vehicle with your driver name.
            </p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-4 gap-5 mb-6">
              <div className="bg-white p-6 rounded-2xl shadow">
                <p className="text-gray-500">Assigned Truck</p>
                <h3 className="text-2xl font-bold text-blue-600">
                  {currentDelivery?.vehicle?.vehicleNumber || "Not Assigned"}
                </h3>
                <p className="text-sm text-gray-500 mt-2">
                  Driver: {name}
                </p>
              </div>

              <div className="bg-white p-6 rounded-2xl shadow">
                <p className="text-gray-500">Today's Deliveries</p>
                <h3 className="text-2xl font-bold text-blue-600">
                  {driverDeliveries.length}
                </h3>
              </div>

              <div className="bg-white p-6 rounded-2xl shadow">
                <p className="text-gray-500">Current Route</p>
                <h3 className="text-lg font-bold text-blue-600">
                  {currentDelivery
                    ? `${currentDelivery.pickupLocation} → ${currentDelivery.dropLocation}`
                    : "No Route"}
                </h3>
              </div>

              <div className="bg-white p-6 rounded-2xl shadow">
                <p className="text-gray-500">Delivery Progress</p>
                <h3 className="text-2xl font-bold text-blue-600">
                  {currentDelivery?.status || "No Delivery"}
                </h3>
              </div>
            </div>

            <div className="bg-white rounded-2xl shadow p-6">
              <h3 className="text-xl font-bold mb-4">Delivery Timeline</h3>

              <div className="flex justify-between items-center">
                <div className="text-center">
                  <div className="w-5 h-5 rounded-full bg-blue-500 mx-auto"></div>
                  <p className="mt-2 text-sm">Pickup</p>
                </div>

                <div className="flex-1 h-1 bg-gray-200 mx-3">
                  <div
                    className={`h-1 ${
                      currentDelivery?.status === "Pending"
                        ? "w-1/4 bg-blue-500"
                        : currentDelivery?.status === "In Transit"
                        ? "w-2/3 bg-yellow-500"
                        : "w-full bg-green-500"
                    }`}
                  ></div>
                </div>

                <div className="text-center">
                  <div
                    className={`w-5 h-5 rounded-full mx-auto ${
                      currentDelivery?.status === "In Transit" ||
                      currentDelivery?.status === "Delivered"
                        ? "bg-yellow-500"
                        : "bg-gray-300"
                    }`}
                  ></div>
                  <p className="mt-2 text-sm">On Route</p>
                </div>

                <div className="flex-1 h-1 bg-gray-200 mx-3">
                  <div
                    className={`h-1 ${
                      currentDelivery?.status === "Delivered"
                        ? "w-full bg-green-500"
                        : "w-0"
                    }`}
                  ></div>
                </div>

                <div className="text-center">
                  <div
                    className={`w-5 h-5 rounded-full mx-auto ${
                      currentDelivery?.status === "Delivered"
                        ? "bg-green-500"
                        : "bg-gray-300"
                    }`}
                  ></div>
                  <p className="mt-2 text-sm">Delivered</p>
                </div>
              </div>

              <div className="mt-6">
                <h3 className="text-xl font-bold mb-3">
                  Welcome, {name}
                </h3>
                <p>
                  You can view your assigned deliveries and track current route
                  from the Tracking page.
                </p>
              </div>
            </div>
          </>
        )}
      </div>
    );
  }

  const cards = [
    ["Total Vehicles", data.vehicles.length],
    [
      "Active Deliveries",
      data.deliveries.filter((d) => d.status === "In Transit").length,
    ],
    [
      "Pending Deliveries",
      data.deliveries.filter((d) => d.status === "Pending").length,
    ],
    ["Total Orders", data.orders.length],
    [
      "Completed Orders",
      data.deliveries.filter((d) => d.status === "Delivered").length,
    ],
  ];

  return (
    <div>
      <div className="bg-gradient-to-r from-blue-700 via-slate-800 to-slate-900 rounded-3xl p-8 mb-8 text-white shadow-xl">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-5xl font-extrabold mb-2">
              Transport Dashboard
            </h1>

            <p className="text-blue-100 text-lg">
              Real-Time Fleet, Delivery & Logistics Monitoring
            </p>
          </div>

          <div className="text-right">
            <p className="text-sm text-gray-300">Welcome Back</p>
            <h3 className="text-2xl font-bold">{name}</h3>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-5 gap-5">
        {cards.map(([title, value]) => (
          <div
            className="bg-white rounded-2xl shadow-lg hover:scale-105 transition p-6"
            key={title}
          >
            <p className="text-gray-500">{title}</p>
            <h3 className="text-4xl font-bold text-blue-600 mt-2">
              {value}
            </h3>
          </div>
        ))}
      </div>

      <div className="bg-white rounded-2xl shadow p-6 mt-6">
        <h3 className="text-xl font-bold mb-3">Project Workflow</h3>
        <p>
          Admin creates orders, schedules deliveries, assigns vehicles and
          drivers, while managers monitor operations and drivers complete
          deliveries.
        </p>
      </div>
    </div>
  );
}